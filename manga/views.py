...
from .models import Manga, Chapters, Search
...
def collection(request):
    ...
    if request.GET.get("searchcoll"):
        mangas = Manga.objects.filter(reader=user, name__icontains=request.GET.get('searchcoll'))
        search = True
    else:
        ...
        search = False
        mangas = Manga.objects.filter(reader=user)
...
@login_required(login_url='login')
def index(request):
    return render(request, 'manga/main.html')
...
def new_search(request):
    ...
    user = request.user.profile
    Search.objects.filter(reader=user).delete()

    my_scraper = Scraper(user)
    my_scraper.search(search)

    search_manga = Search.objects.filter(reader=user)
    collection_manga = Manga.objects.filter(reader=user).values_list('manga_id', flat=True)
...
def add_to_coll(request):
    ...
    if request.method == "POST":
        ...
        search_manga = Search.objects.get(manga_id=pk)
        my_scraper = Scraper(user)
        my_scraper.follow(search_manga)

        if not Manga.objects.filter(name=search_manga.name, reader=user).exists():
            new_manga = Manga(reader=search_manga.reader, name=search_manga.name, img_url=search_manga.img_url,
                              manga_url=search_manga.manga_url, manga_id=search_manga.manga_id)
            new_manga.save()
            added = True
